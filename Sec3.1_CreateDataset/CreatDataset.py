...
#
    ...
    #
    # OTF_bsis=np.abs(approx_complex_matrix[69,:,:])
    #
    ...
    #
    # #PSF网格大小为d_u=d_fx*namuda*f=4.197*10^-6m,OTF网格大小为d_m=1/(N*d_u)=1.8614*10^-3μm^-1
    # #PSF grid size:d_u=d_fx*namuda*f=4.197*10^-6m=4.197μm， OTF grid size:d_m=1/(N*d_u)=1.8614*10^-3μm^-1
    #
    #
    # # 物理参数设置， Physical Parameter Setup
    # grid_size = 1.861  # 每个网格的微米数，Micrometers per grid
...
########################################################################################################
    # #PSF网格大小为d_u=d_fx*namuda*f=4.197*10^-6m,OTF网格大小为d_m=1/(N*d_u)=1.8614*10^-3μm^-1
    # #PSF grid size:d_u=d_fx*namuda*f=4.197*10^-6m， OTF grid size:d_m=1/(N*d_u)=1.8614*10^-3μm^-1
...
#
#     # pca = joblib.load('/home/aiofm/PycharmProjects/MyKANNet/15e-16Cn2Data/New_pca_model-70.pkl')
#     # approx_complex_matrix=pca.inverse_transform(PCA_70_coff[4:5])
#
#     dim_x = np.arange(0, 70)
#
#     plt.figure(figsize=(14, 10), dpi=120)
#
#     # 绘制折线
#     plt.plot(dim_x, PCA_70_coff[4], color='blue', linewidth=1.2, zorder=1)
#     # 绘制散点（层级更高，覆盖在线上方）
#     plt.scatter(dim_x, PCA_70_coff[4], color='red', s=18, alpha=0.8, zorder=2)
#
#     # 4. 图表美化设置
#     plt.xlabel("Vector Dimension (0~79)", fontsize=20)
#     plt.ylabel("Value", fontsize=20)
#     plt.title("80-Dimensional Vector Value Distribution", fontsize=12, pad=12)
#     plt.xticks(np.arange(0, 71, 10), fontsize=20)
...
#     plt.show()
    ...
    # # 物理参数设置， Physical Parameter Setup
    # grid_size = 1.861  # 每个网格的微米数，Micrometers per grid
    # total_pixels = 128  # 总像素数，Total number of pixels
    # half_pixels = total_pixels // 2  # 半像素数，Half the number of pixels
    #
    # # 计算从中心向两边扩展的实际物理范围，Calculate the actual physical range extending from the center to both sides
    # # 中心为原点(0,0)，向左右上下各扩展的距离，With the center as the origin (0,0), calculate the extension distances to the left, right, top, and bottom.
    ...
    #
    # # 绘制图像，设置坐标范围以中心为基准向两边扩展，Plot the image and set the coordinate range to extend symmetrically from the center.
    # im = ax.imshow(np.abs(approx_complex_matrix),
    #             extent=[-max_extent, max_extent, -max_extent, max_extent],
    #             origin='lower',
    #             cmap='viridis')
    #
    # # im = ax.imshow(np.abs(OTF[4,:,:]),
    # #                 extent=[-max_extent, max_extent, -max_extent, max_extent],
    # #                 origin='lower',
    # #                 cmap='viridis')
    #
...
# 1. 读取 npz 数据
# =========================
file_path = "/media/aiofm/F/20250723_k-fold-cross-validation-KAN/10^7DataSet/10^7_aj_compressed.npz"

data = np.load(file_path)

# 假设数据保存在某个 key 里（常见：'arr_0' 或 'data'）
print(data.files)  # 先看有哪些键

X = data[data.files[0]]  # 如果不确定key，就先用第一个

print("数据形状:", X.shape)  # 应该是 (5000000, 33)
...
# 3. 保存模型
# =========================
model_path = "/media/aiofm/F/20250723_k-fold-cross-validation-KAN/kmeans_models/kmeans_k20_model.joblib"
...
# 配置路径 & 超参
#=========================================================
input_path = '/media/aiofm/F/20250723_k-fold-cross-validation-KAN/10^7DataSet/10^7_aj_compressed.npz'
label_base = '/media/aiofm/F/20250723_k-fold-cross-validation-KAN/10^7DataSet'
model_path = "/media/aiofm/F/20250723_k-fold-cross-validation-KAN/kmeans_models/kmeans_k20_model.joblib"
output_base = '/media/aiofm/F/20250723_k-fold-cross-validation-KAN'

batch_size = 100000
n_splits = 8          # 固定8折
fold_id = 0           # 选择当前执行第几折：0 ~ 7
# 单簇内划分比例：训练70% / 验证10% / 测试20%
train_ratio = 0.7
val_ratio   = 0.1
test_ratio  = 0.2
...
for i in range(0, total_num, batch_size):
    end = min(i + batch_size, total_num)
    clusters[i:end] = kmeans.predict(X[i:end])
    print(f"{i}/{total_num}")
...
y_list = []
for i in range(1, 26):
    path = f"{label_base}/10^7_PCA70_coff_compressed_batch_{i}.npz"
    d = np.load(path)
    y_list.append(d[d.files[0]])

Y = np.vstack(y_list)
print("Y shape:", Y.shape)

# =========================================================
# 核心函数：每个簇内部独立划分 训练/验证/测试
# 逐簇划分后，聚合所有簇结果得到全局数据集
# =========================================================
def split_by_each_cluster(all_clusters, tr, va, te):
    """
    每个聚类簇内部按比例划分，最后聚合所有簇的结果
    :return: 全局训练索引、验证索引、测试索引
    """
    all_train_idx = []
    all_val_idx = []
    all_test_idx = []
    unique_cls = np.unique(all_clusters)

    for cls in unique_cls:
        # 获取当前簇所有样本索引
        cls_mask = all_clusters == cls
        ...
        split1 = int(n * tr)
        split2 = split1 + int(n * va)

        train_cls = cls_idx[:split1]
        val_cls = cls_idx[split1:split2]
        test_cls = cls_idx[split2:]

        # 追加到全局列表
        all_train_idx.extend(train_cls)
        all_val_idx.extend(val_cls)
        all_test_idx.extend(test_cls)
...
# 保存函数
# =========================================================
def save_split(X, Y, idx, save_path):
    np.savez_compressed(save_path, X=X[idx], Y=Y[idx])

# =========================================================
# 8. 执行划分并保存
# =========================================================
print(f"===== Start processing Fold {fold_id + 1} (id={fold_id}) =====")

# 逐簇划分，聚合所有簇结果
train_idx, val_idx, test_idx = split_by_each_cluster(clusters, train_ratio, val_ratio, test_ratio)

print(f"Total samples: {len(X)}")
print(f"Train: {len(train_idx)}, Val: {len(val_idx)}, Test: {len(test_idx)}")

# 创建保存目录
base_dir = os.path.join(output_base, f"{fold_id+1}-fold")
...
save_split(X, Y, train_idx, os.path.join(train_path, "train.npz"))

print("Saving val set...")
save_split(X, Y, val_idx, os.path.join(val_path, "val.npz"))

print("Saving test set...")
save_split(X, Y, test_idx, os.path.join(test_path, "test.npz"))
...
def npz_to_h5(npz_path, h5_path):
    """
    将单个 npz 文件转为 h5 文件
    约定 npz 内键：X, Y
    """
    print(f"Converting: {npz_path} -> {h5_path}")
    with np.load(npz_path, mmap_mode="r") as npz_file:
        X_data = npz_file["X"]
        Y_data = npz_file["Y"]

        with h5py.File(h5_path, "w") as h5_file:
            # 创建数据集，开启压缩（可选，平衡体积/速度）
            h5_file.create_dataset("X", data=X_data, compression="gzip", compression_opts=4)
            h5_file.create_dataset("Y", data=Y_data, compression="gzip", compression_opts=4)
    print(f"Convert done: {h5_path}\n")
...
for split in splits:
    npz_file = os.path.join(data_root, split, f"{split.lower()}.npz")
    h5_file = os.path.join(data_root, split, f"{split.lower()}.h5")
    if os.path.exists(npz_file):
        npz_to_h5(npz_file, h5_file)
    else:
        print(f"Warning: {npz_file} not found, skip.")
